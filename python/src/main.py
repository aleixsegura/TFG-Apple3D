...
def transform_points(lidar_points: NDArray[np.float64], imu_data: NDArray[np.float64], 
                                timestamps_seconds: NDArray[np.float64], gnss_x, gnss_y, gnss_z):    
    ...
    x_transformed = (x * cos_yaw * cos_pitch - y * sin_yaw + z * cos_yaw * sin_pitch) + gnss_y
    
    y_transformed = (x * sin_yaw * cos_pitch + y * cos_yaw + z * sin_yaw * sin_pitch) + gnss_x
    
    z_transformed = (-x * sin_pitch + z * cos_pitch) + gnss_z
    
    transformed_points = lidar_points.copy()
    transformed_points[:, 1:4] = np.column_stack((x_transformed, y_transformed, z_transformed))
...
if __name__ == '__main__':
    ...
    print(f'Duration of the program = {elapsed} seconds.')
